...
#include <numeric> // Hincoin: needed for std::accumulate
...
uint256 hashGenesisBlock("0x29f7a296c47443c1e661bd8cd7f010e43d523513c0398c3b325f3899145efcef");
...
bool nUsingHincoinStochasticUpdate = false;
unsigned int nCoinCacheSize = 5000;
int64 nChainStartTime = 1393448369; // Line: 2815


int64 nStochasticStartTime = 1396362673; //HinCoin: At this time, start the stochastic  update of the nValue.
int64 nTwoWeeksTime = 1209600; // HinCoin: This is the length of time (in seconds) in a 2 weeks span.
int64 nHincoinLastStochasticTimeUpdate = 0;
...
unsigned char GetNfactor(int64 nTimestamp) {
    ...
    if(nUsingHincoinStochasticUpdate)
    {
        int wholeN = (int) nHincoinRetargetN;
        return min(max(minNfactor,(unsigned char)wholeN),maxNfactor);
    }
    int l = 0;
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // hincoin: 840k blocks in ~4 years
...
static const int64 nTargetSpacing =3 * 60; // hincoin: 3 minute blocks
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // doens't matter.. its depreicated.. only based on KGW from now on
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
    ...
    if (BlockLastSolved == NULL || BlockLastSolved->nHeight == 0 || (uint64)BlockLastSolved->nHeight < PastBlocksMin) {return bnProofOfWorkLimit.GetCompact(); }
       // printf("Starting the KGW algorithm\n");
        ...
        if(BlockLastSolved->nHeight == PastBlocksMin)
        {
...
{
    double alphaPoint = .15;
    double MaxBlocksToCount = 6720; // expected number of blocks after 2 weeks of HinCoin mining
    const CBlockIndex* blockReading = pindexLast;
    double runningAverage = 0.0;
    int i= 1;
    for(; blockReading && blockReading->nHeight > 0; ++i)
    {
        if(i > MaxBlocksToCount) break;
        runningAverage += blockReading->GetBlockTime();
        blockReading = blockReading->pprev;

    }
    runningAverage /= (i * 180);
    nHincoinRetargetN = nHincoinRetargetN + (alphaPoint * (1-runningAverage));
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if(pindexLast->nTime >= nStochasticStartTime)
    {
        nHincoinRetargetN = GetNfactor(pindexLast->nTime);
        nUsingHincoinStochasticUpdate = true;
    }
    /*
            I commented the following block out completely for testing reasons... this is just to verify that KGW is doing its job!!
    ...
    */
        ...
        */
        ...
        //printf("Nheight: %i\n",pindexLast->nHeight);
        unsigned int nextDifficulty = GetNextWorkRequired_V2(pindexLast, pblock);
        if(pindexLast->nHeight == 96)
        {
            printf("KGW officially started!");
            printf("The difficulty is set at: %ui\n",nextDifficulty);
        }
        if(nUsingHincoinStochasticUpdate && ( pindexLast->nTime == nStochasticStartTime || pindexLast->nTime - nTwoWeeksTime == nHincoinLastStochasticTimeUpdate) )
        {
            updateN(pindexLast);
            nHincoinLastStochasticTimeUpdate = pindexLast->nTime;
        }
        return  nextDifficulty;// KGW
}
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        pchMessageStart[2] = 'r';
        pchMessageStart[3] = 't';
        hashGenesisBlock = uint256("0x29f7a296c47443c1e661bd8cd7f010e43d523513c0398c3b325f3899145efcef");
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nNonce = 1070457;
        block.nTime = 1393463437;
